...
來源：SRS §2.1、US-001–004、資料庫關聯圖，以及 2026-09-18 使用者確認的開卡基準、只輸入變化、初始／變化／總計快照、當前狀態累計與 C02 魔法物品來源規則。新增規則為目標，尚未實作。玩家名稱來源、完整職業／等級變化契約、靈魂幣欄位尚待 C05–06 及欄位矩陣補齊；C04 的同日排序、開卡狀態保存方式、變化造成不合法狀態及舊資料基準仍待決策。欄位上限源自 SRS，是否所有入口均實作尚待驗證。
