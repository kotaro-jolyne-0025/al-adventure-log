...
| 金幣合計 | 數字（小數點後 2 位） | ❌ | **系統計算（起始 + 冒險變化 + 休整期變化），不可手動修改；表單即時顯示合計，詳情顯示初始／變化／合計** |
| 起始休整期天數 | 整數 | ❌ | 系統依日期前置快照計算，僅供計算與詳情檢視；新增／編輯表單不顯示 |
| 休整期天數冒險變化 | 整數 | ❌ | 正負值均可 |
| 休整期天數休整期變化 | 整數 | ❌ | 正負值均可 |
| 休整期天數合計 | 整數 | ❌ | **系統計算（起始 + 冒險變化 + 休整期變化），不可手動修改；表單即時顯示合計，詳情顯示初始／變化／合計** |
| 起始魔法物品數 | 整數 | ❌ | 系統依日期前置快照計算，僅供計算與詳情檢視；新增／編輯表單不顯示 |
| 魔法物品冒險變化 | 整數 | ❌ | 正負值均可 |
| 魔法物品休整期變化 | 整數 | ❌ | 正負值均可 |
| 魔法物品合計 | 整數 | ❌ | **系統計算（起始 + 冒險變化 + 休整期變化），不可手動修改；表單即時顯示合計，詳情顯示初始／變化／合計** |
| 冒險備註 | 長文字 | ❌ | 自由填寫 |
| 靈魂幣使用 | 文字 | ❌ | 自由填寫 |
...
| 功能 | 說明 |
| --- | --- |
| 新增冒險記錄 | 表單只輸入本次變化，不顯示資源初始值；合計即時由系統計算，詳情仍呈現初始／變化／合計；採用獨立資源卡片（金幣/休整期/魔法物品）與即時合計徽章；支援記錄永久魔法物品、消耗品及故事獎勵 |
